    ...
    Game.profiler = {
        ...
        restart() {
            if (Profiler.isProfiling()) {
                ...
                setupMemory(Memory.profiler.type, duration, filter);
            }
        },
        reset: resetMemory,
...
function setupMemory(profileType, duration, filter) {
    ...
    const disableTick = Number.isInteger(duration) ? Game.time + duration + 1 : false;
...
const Profiler = {
    ...
    output(lines = 30, sort = 'totalTime') {
        ...
        return [].concat(header, Profiler.lines(sort).slice(0, lines), footer).join('\n');
    },

    lines(sort = 'totalTime') {
    ...
    prototypes: [
        ...
        { name: 'Shard', val: Shard },
        { name: 'Traveler', val: Traveler },
        // { name: 'Flag', val: Flag },
    ],
